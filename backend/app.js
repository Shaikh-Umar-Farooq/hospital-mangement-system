...
app.get('/docssched', (req, res) => {
  connection.query("select doctor,day,starttime as 'Start time', endtime as 'End time',breaktime as 'Break time' from Schedule join DocsHaveSchedules on Schedule.id=DocsHaveSchedules.sched;", (err, results, fields) => {
      if (err) throw err;
      res.send(results);
  });
});
